...
private:
  ...
  unordered_map<string,
                function<void(int, const HTTP_Request &, HTTP_Response &)>>
      getRoutes;
  ...
  void httpRequestParser(int clientSocket, string &method, string &path,
    ...
    int space_count = 0;
    // NOTE: now main buffer is the string which contains all the request
...
public:
  ...
  void get(const string &path,
           function<void(int, const HTTP_Request &, HTTP_Response &)> handler) {
    // key-> path/route value- function which will take clientsocket fd as
    // argument
