...
void HTTP_SERVER::httpRequestParser(int clientSocket, std::string &method,
    ...
    if (headerPos == std::string::npos) {
        ...
        while (mainBuffer.size() <= 32000) {
            ...
            size_t bytesRead =
                read(clientSocket, tempBuffer, sizeof(tempBuffer));
            if (bytesRead <= 0) {
                // FIXME: if any errors comes in this we need to do something on
                // the response object
    ...
    */
    ...
    if (request.headers.count("Content-Length")) {
        ...
        if (contentLength > maxBodyLength) {
            // FIXME: this should be done after creating the response object
            // reject this request
            response.setStatus(404, "Bad Request");
            response.setContentType("text/plain");
            std::string res = response.genResponse();
